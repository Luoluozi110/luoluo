  ...
  showAbilityPanel(game) {
    ...
    const render = (notice = '') => {
      ...
      box.innerHTML = `<header class="ability-header"><div class="mtitle"><h2>三功修习</h2><button class="btn btn-ink" data-close>关闭</button></div>
        ...
        <div class="strategy-progress"><span>构思进度 <b>${fmt(fb.strategyRemainder)}/${fb.strategyProgressNeed}</b></span><progress aria-label="构思换算进度" value="${fb.strategyRemainder}" max="${fb.strategyProgressNeed}"></progress><span>下阶段进度 +${fmt(fb.strategyIncome)}；每 ${fb.strategyProgressNeed} 进度换 1 构思</span></div><nav class="ability-tabs" aria-label="修习分区">${Object.entries(names).map(([id, name]) => `<button data-section="${id}" aria-pressed="${section === id}">${name}</button>`).join('')}</nav><div class="ability-notice" role="status">${esc(notice)}</div></header>
