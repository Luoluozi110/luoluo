...
console.log('== 当前研修方向：推进进度，不直接灌属性 ==');
{
  ...
  assert.equal(game.s.abilityState.study.progress.shi, 1.44, '强化博览后诗力研修进度按学力 +1.44');
...
console.log('== 心得已满：收益转为临场研修 ==');
{
  ...
  assert.equal(game.s.abilityState.study.progress.si, 1.44, '强化博览后满额心得自动转为思力研修 +1.44');
