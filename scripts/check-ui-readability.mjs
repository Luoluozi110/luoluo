...
try {for (const [width,height] of [[1440,900],[1280,720],[768,1024],[390,844],[844,390],[720,450]]) {
 ...
 await p.waitForTimeout(400); await p.screenshot({path:`docs/ui-release-20260910/hud-${width}.png`});
 ...
 await p.waitForTimeout(400); await p.screenshot({path:`docs/ui-release-20260910/ability-${width}.png`});
 ...
 await p.screenshot({path:`docs/ui-release-20260910/manuscript-${width}.png`});
 ...
 await p.waitForTimeout(500); await p.screenshot({path:`docs/ui-release-20260910/talent-${width}.png`});
 ...
 await p.screenshot({path:`docs/ui-release-20260910/dice-${width}.png`});
...
}fs.writeFileSync('docs/ui-release-20260910/measurements.json',JSON.stringify({baseline:'UI implementation on audit baseline c8ad1fd',mode:'Local production components with simulated midgame state; cloud JSON served from checkout',results},null,2));console.log(JSON.stringify(results));}
