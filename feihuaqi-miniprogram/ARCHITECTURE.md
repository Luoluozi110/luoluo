...
| 第 2 周 | 真实交互 + 云开发接入 | **部分达成**：答题 / 奇遇 / 天象 / 博闻 / 文心替换 / 战斗台六步已接好并自检通过；云开发联调与真机验证待做 |
...
玩家决策的本质问题是：引擎 `await` 一个返回值才能继续，而这个值来自人。

解法是让适配器发一个 `request` 事件并挂起 Promise，页面渲染对话框、玩家操作后调用 `evt.resolve(value)`：

```
引擎 await ui.showQuiz(...)  →  适配器 emit request + 挂起  →  页面 setData 渲染
                                                          →  玩家点选
                                                          →  evt.resolve({index, timedOut})
                                                          →  引擎继续
```

关键约定：**sink 收到 request 时返回 `true` 表示接管，返回 `false`/`undefined` 则适配器回退自动应答。**
这条兜底让「界面还没做的部分」不会把流程卡死，可以边做边接。

另一条纪律：引擎的原始对象（如战斗 `session`）通过事件的 `raw` 字段传递，
**页面挂在实例上而不进 `data`**——它不可序列化，进 `setData` 会炸。

### 10.7 已接好的界面

| 界面 | 引擎回调 | 玩家看到的 |
|---|---|---|
| 答题 | `showQuiz` | 情境 + 题干 + 选项，落笔作答 |
| 奇遇 | `showEvent` | 事件描述 + 选项，就此决定 |
| 天象 | `showSky` | 天象说明，记下了 |
| 博闻抉择 | `showBowenChoice` | 专攻一体 / 以学驭战 / 兼收并蓄 |
| 文心替换 | `askReplaceTalent` | 新得文心与已有文心，定夺（不选即放弃） |
| 战斗台 | `runBattle` | 六步：遭遇 → 选文体 → 选文风 → 掷灵感骰（可追加）→ 算分对决 |

战斗台的每一步都直接读引擎 session 的实时数据：
`styleScore` 给出各文体底分，`affinityOf` 给出文风相性，
`previewDiceScore` 预览骰面得分，`extraDiceCost` 算追加骰的灵感消耗。
追加骰走 `session.spendExtraDice(cost)`，灵感不足时拒绝掷骰。

### 10.8 验收

```bash
node test/smoke-interactive.mjs   # 模拟玩家接管每个决策，走完整局
```

实测一局：49 次玩家决策中 42 次由页面接管（答题 8、论战 12、奇遇 16），
7 次走兜底（尚未实装的支线、阶段弹窗、殿试开场）。

策略生效的佐证：自动应答时战绩常为「0 胜 15 负」，
改为挑最高相性文风后变成「5 胜 5 平 8 负」——界面选择确实在影响结果。
...
| 界面 | 引擎回调 | 玩家看到的 |
|---|---|---|
| 名胜 | `askScenic` | 访胜抽签 / 入世另行 / 行路凝心 / 翻看手记 / 离开，五个去向 |
| 三心择一 | `chooseScenicTalent` | 三枚未拥有文心择一，`-1` 为放弃 |
| 晋阶 | `showStageChange` | 阶段推进说明，并为本章择一「开卷句」 |
| 殿试开场 | `showPalaceIntro` | 本场题目与开卷句选择 |

`showPlannedMovePrompt` 引擎当前并未调用，适配器保留空实现。
