// 文心棋小程序 · 入口
//
// 全工程统一 ES Module 语法，避免 require / import 混用带来的编译歧义。
import storage from './utils/storage.js';
import { ensureEnv, silent } from './utils/cloud.js';

// 在业务代码执行前把 localStorage 挂到全局，
// 使从 H5 搬来的引擎存档逻辑（album.js / save.js 等）无需任何改动即可工作。
if (typeof globalThis.localStorage === 'undefined') {
  globalThis.localStorage = storage;
}

// 主包配置不再逐个 require JSON：scripts/build-engine.mjs 已把
// config/*.json 编译成 engine/embed-config.js（纯 ES Module），由 engine-runtime 统一装配。
...
App({
  ...
  login() {
    ...
    return silent('authLogin', { channel: this.globalData.channel })
      .then((res) => {
