...
(function (global) {
  ...
  function openOverlay(id) {
    ...
    // 富文本增强：弹窗内若有 data-rich 文本域（含动态构建的），立即包裹工具栏/素材/预览
    try { const rt = global.Common && global.Common.richText; if (rt) rt.enhanceAll(el); } catch (_) {}
    setTimeout(() => {
  ...
  function refreshWorkspaceUI() {
    updateWorkspaceSummary();
    // 初始渲染完成后，把各标签页里出现的 data-rich 文本域统一增强为富文本编辑器
    try { const rt = global.Common && global.Common.richText; if (rt) rt.enhanceAll(document); } catch (_) {}
  }
  ...
  function routeImport(data, mode) {
    ...
    if (Array.isArray(data)) {
      ...
      if (!routed) { alert("文件不含 questions / events / talents / talent-upgrade / npcs / sidequest-npcs / affinity / synergies / board / sky / album / schools / grades / narrative 字段。"); return; }
