...
(function (global) {
  ...
  function renderList() {
    ...
    list.innerHTML = arr.length ? arr.map(c => { const i = state.cards.indexOf(c); return `<div class="q-card"><div class="meta"><span class="q-id">${C.esc(c.id)}</span><span class="badge r-common">${C.esc(c.unlock.type)}</span><span class="badge r-common">${c.branches.length ? "成长×" + c.branches.length : "旧式"}</span></div><div class="q-main"><p class="q-name">${C.esc(c.name)}</p><div class="q-tags"><span class="t">解锁：${C.esc(unlockText(c.unlock))}</span><span class="t">奖励：${C.esc(c.rewardDesc || rewardText(c.reward))}</span></div><div class="q-opts">${C.esc(c.text.slice(0, 150))}</div></div><div class="q-actions"><button class="btn sm" data-album-preview="${i}">预览</button><button class="btn sm" data-album-edit="${i}">编辑</button><button class="btn sm danger" data-album-del="${i}">删除</button></div></div>`; }).join("") : `<div class="empty">没有符合条件的传世名篇</div>`;
  }
  ...
  function bind() {
    ...
    document.getElementById("albumlist").addEventListener("click", e => { const t = e.target; if (t.dataset.albumEdit != null) openEditor(Number(t.dataset.albumEdit)); else if (t.dataset.albumDel != null) { if (confirm("确定删除这张传世名篇？")) { state.cards.splice(Number(t.dataset.albumDel), 1); save(); renderList(); } } else if (t.dataset.albumPreview != null) { const c = state.cards[Number(t.dataset.albumPreview)]; document.getElementById("albumPreviewBody").innerHTML = `<h3>${C.esc(c.name)}</h3><p>${C.esc(c.text)}</p><p>解锁：${C.esc(unlockText(c.unlock))} · 奖励：${C.esc(c.rewardDesc || rewardText(c.reward))}</p><p>成长：${C.esc(JSON.stringify(c.growth || {}))}</p><pre style="white-space:pre-wrap">${C.esc(JSON.stringify(c.branches || [], null, 2))}</pre>`; C.openOverlay("albumPreviewOverlay"); } });
