...
## 使用方式
打开正式编辑器：<https://luoluozi110.github.io/luoluo/feihua-editors/>（本地调试则打开 `feihua-editors/index.html`，或运行 `npm run editor:open`）。叙事型文本域（奇遇正文/抉择、文心正文、天象正文、叙事文案标签）上方出现工具栏，点「素材」展开游戏素材面板，右侧实时显示游戏风格预览。窄屏自动单列堆叠。

## 后续可选
- 游戏端加一个轻量 Markdown/令牌渲染层（HTML 先转义再套规则），让编辑器产出的富文本在游戏内真正呈现。
- 编辑器已经随 GitHub Pages 同仓部署；正式入口固定为 <https://luoluozi110.github.io/luoluo/feihua-editors/>。CloudStudio 当前仅作为游戏预览分享页，不再维护单独的编辑器分享链接。
